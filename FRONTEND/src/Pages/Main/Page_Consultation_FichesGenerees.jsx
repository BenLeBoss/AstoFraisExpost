...
function Consultation_FichesGenerees(){
    ...
    useEffect(() => {
        ...
        if (Groupe === 'ASTORIA')
            load_fiches_generees_ASTORIA();

    }, [])


    const load_fiches_generees_ASTORIA = async() => {
        try{
            setClients([]);
            setNombreFiches(0);
            
            setLoading(true);
            const res = await fetch('/api/client/consultation/fichesgenerees', {
                method: 'GET',
                headers: { 
    ...
    const load_fiches_generees_INSINIA = async(Entite) => {
        try{
            ...
            setLoading(true);
            const res = await fetch('/api/client/consultation/fichesgenereesinsinia', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    'Authorization' : `Bearer ${token}`
        ...
        <div className='Page_Consultation_FichesGenerees-root'>
            ...
            <div className='header'>
                <div className='header-titre'>
                    <h1> Fiches générées pour les clients suivants </h1>
                </div>
                <div className='container-ligne-listepartenaires'>
                    <div className='header-ligne'>
                        <label> {(NombreFiches ?? 0) + " Fiches ont été générées"}  </label>
                    </div>
...
export default Consultation_FichesGenerees
